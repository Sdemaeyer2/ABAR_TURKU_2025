...
draws_array <- as_draws_array(M3b)

# Calculate Rhat directly from array
rhat_vals <- rhat(draws_array)

# Convert to numeric named vector
rhat_numeric <- as.numeric(rhat_vals)
names(rhat_numeric) <- names(rhat_vals)

# Remove potential NA values explicitly
rhat_numeric <- rhat_numeric[!is.na(rhat_numeric)]

# Finally, plot with bayesplot
mcmc_rhat(rhat_numeric, size = 2) +
  yaxis_text(hjust = 1)
